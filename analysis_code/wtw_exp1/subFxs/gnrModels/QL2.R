...
QL2 = function(paras, condition_, scheduledWait_, normResults){
  # default settings 
  iti = 2
  load("expParas.RData")
  ...
  for(tIdx in 1 : nTrial) {
    ...
    if(tIdx < nTrial){
      ...
      Gts =  gamma ^ (T - tWaits) * (trialEarnings + V)
      # only update value functions before time t = T
      updateFilter = tWaits <= T 
      # update Qwaits
      Qwaits[updateFilter] = Qwaits[updateFilter] + LR * (Gts[updateFilter] - Qwaits[updateFilter])
      
      # calculate expected returns for t == 0 and update V
      Gt =  gamma ^ (T - (-iti)) * (trialEarnings + V)
      V = V + LR * (Gt - V)
