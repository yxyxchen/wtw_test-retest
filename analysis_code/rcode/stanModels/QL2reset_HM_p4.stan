...
parameters {
  ...
  vector[4] mu; # mean of raw parameters
  vector<lower=0>[4] sigma; # std of raw parameters
...
transformed parameters{
  ...
  for(sIdx in 1 : S){
    alpha[sIdx] = inv_logit(raw_alpha[sIdx]);
    alphaU[sIdx] = min([alpha[sIdx] * exp(raw_nu[sIdx]), 1]'); // 
    nu[sIdx] = alphaU[sIdx] / alpha[sIdx];
    tau[sIdx] = exp(raw_tau[sIdx]);
    gamma[sIdx] = inv_logit(raw_gamma[sIdx]);
    eta[sIdx] = exp(raw_eta[sIdx]);
  }
}
model {
  ...
  raw_gamma ~ normal(mu[3], sigma[3]);
  raw_eta ~ normal(mu[4], sigma[4]);
...
generated quantities {
  ...
  real mu_raw_gamma = mu[3];
  real mu_raw_eta = mu[4];
  
  real sigma_raw_alpha = sigma[1];
  real sigma_raw_tau = sigma[2];
  real sigma_raw_gamma = sigma[3];
  real sigma_raw_eta = sigma[4];
