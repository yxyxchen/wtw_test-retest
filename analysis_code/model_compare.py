...
for sess in [1, 2]:
    a.loc[a[("sess", "")] == sess].iloc[:,2:].to_csv(os.path.join("..", "figures", expname, "waic_compare_sess%d.csv"%sess), index = None, header = None)
    a.loc[a[("sess", "")] == sess].iloc[:,2:].apply(lambda x: np.quantile(x, 0.25), axis = 0)
    a.loc[a[("sess", "")] == sess].iloc[:,2:].apply(np.median, axis = 0)
    a.loc[a[("sess", "")] == sess].iloc[:,2:].apply(lambda x: np.quantile(x, 0.75), axis = 0)
    a.loc[a[("sess", "")] == sess].iloc[:,2:].apply(np.argmin, axis = 1).value_counts()
    np.quantile(a.loc[a[("sess", "")] == sess, "diff"], [0.25, 0.5, 0.75])
    a.shape[0]
