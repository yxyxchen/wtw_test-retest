...
def group_quality_check(sess, plot_quality_check = False):
	...
	# excluded.to_csv(os.path.join("..", "analysis_results", "excluded", "excluded_participants_sess%d.csv"%sess), index = False)
...
def parse_group_selfreport(sess, isplot):
	...
	for i, row in selfreport.iterrows():
		...
		if isplot and (sess == 1 or (sess == 2 and not np.isnan(row[4]))):
			...
			plt.clf()
	selfdata.to_csv(os.path.join(os.path.join("..", "analysis_results", "selfreport"), "selfreport_sess%d.csv"%sess), index = False)

# Maybe I need a function to load MCQ

############################## load model parameter estimates######################
def load_parameter_estimates(sess, hdrdata, modelname):
	...
	for i, subj_id in enumerate(hdrdata['id']):
		try:
			fit_summary = pd.read_csv(os.path.join("..", "analysis_results", "modelfit", modelname, '%s_sess%d_summary.txt'%(subj_id, sess)), header = None)
		except:
