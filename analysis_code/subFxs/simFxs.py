...
def RL1_learn(Qwaits, Qquit, reward_rate, ts, timeWaited, trialEarnings, paras, empirical_iti = expParas.iti):
...
def RL2_learn(Qwaits, Qquit, reward_rate, ts, timeWaited, trialEarnings, paras, empirical_iti = expParas.iti):
...
def ind_sim(modelname, paras, condition_, blockIdx_, scheduledDelay_, scheduledReward_, empirical_iti = expParas.iti):
	...
	for tIdx in range(nTrial):
		...
		while t < max(expParas.tMaxs):
			...
			if (action == 'wait' and t + expParas.stepsize >= scheduledDelay):
